...
**架构：** 使用单个 `index.html` 文件承载语义化 HTML 和少量内嵌 CSS。PowerShell 验证脚本负责检查本地文件契约，浏览器负责验证电脑与手机尺寸下的实际渲染；部署阶段优先使用当前已登录的静态托管服务。
...
- [ ] **步骤 1：检查可用的托管凭据且不输出秘密信息**

检查已安装部署工具的登录状态。当前项目已有 GitHub 远程仓库，因此优先使用 GitHub Pages；其次选择其他已经登录的静态托管服务。

- [ ] **步骤 2：在不修改页面代码的前提下部署**

将仓库根目录配置为静态站点来源，或把静态目录上传到选定服务，并保持 `index.html` 不变。
